...
public:    
	...
	Keypoint(float x, float y, float scale, float ori) :
	m_x(x), m_y(y),m_scale(scale),m_ori(ori)
	{ m_r = 0; m_g = 0; m_b = 0; }
...
public:
	...
	KeypointWithDesc(float x, float y,float scale, float ori, unsigned char *d) :
	Keypoint(x, y, scale, ori), m_d(d)
	{ }
...
#else
    int m_idx1, m_idx2;
	float m_x1, m_y1;	//µã×ø±ê (pixel)	zj 2011.2.22
	float m_x2, m_y2;
#endif
};
