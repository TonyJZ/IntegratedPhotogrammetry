...
typedef std::pair<unsigned long, unsigned long> MatchIndex;		//匹配像对索引
...
//相邻的像片（有重叠）基准像片的片号记录在MatchTable中的m_match_lists的索引号
class AdjListElem 
{
public:
    ...
    unsigned int m_index;	//片号
    std::vector<KeypointMatch> m_match_list;	//对应点列表
};

typedef std::vector<AdjListElem> MatchAdjList;	//与当前片相邻的列表，一张基准片可能有多张相邻像片，组成链表
...
public:
    ...
    std::vector<PointData> m_point_data;   /* Information about 3D		物方点
					    * points in the scene */
