...
//按照指定参数计算连接平面的平面参数
int _lidCalib_Dll_ lidPlaneFitting_Skaloud(LidMC_Plane *pLidPlane, int nPlane, orsLidSysParam oriParam, double *Lidparam, CalibParam_Type type);
...
void _lidCalib_Dll_ LidStatDis_PointToPlane_Skaloud(LidMC_Plane *pLidPatch, int nPatch, double *Lidparam, bool bInit, orsLidSysParam oriParam,
											CalibParam_Type param_type,double *rms, int nTies, double *pDis=NULL);

class _lidCalib_Dll_ CLidCalib_Skaloud : public CLidarSelfCalibration
{
public:
     CLidCalib_Skaloud();
 	 virtual ~CLidCalib_Skaloud();
