...
void CGridButton::OnLButtonUp(UINT nFlags, CPoint point) 
{
	...
	cell = pEdit->GetGrid()->GetFocusCell();
...
//	if (pEdit->m_pGrid && !cell.IsEmpty() && pEdit->m_editStyle == geEllipsis)  {
		...
		pWnd = pEdit->GetGrid()->GetParent();
		ASSERT(pWnd);
		ASSERT(::IsWindow(pWnd->GetSafeHwnd()));


		dispInfo.hdr.code = VGN_EDITBUTTONPRESS;
		dispInfo.hdr.hwndFrom = pEdit->GetGrid()->m_hWnd;
		dispInfo.hdr.idFrom = pEdit->GetGrid()->GetDlgCtrlID();

		dispInfo.item.nColumn = cell.col;
		dispInfo.item.nRow = cell.row;
