...
//导出像点误差
//返回全部像点的重投影误差均值
double output_image_pts_error(void *adata, int j, int mnp, double *x, double *hx, struct sba_crsm *idxij, int *rcidxs, int *rcsubs)
{
	register int i;
	int nnz, nprojs;
	double *ptr1, *ptr2;
	...
	fp = fopen(gl->pszImgPtResidual, "wt");
	if(fp==NULL)
		return 1e+12;
