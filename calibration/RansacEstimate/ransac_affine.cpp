...
bool EstimateTransform_Affine(myPtPair *param, int ptNum, int &refineNum, double Outlierth, double *Mout) 
{
	int min_matches = 3;
...
//	double RANSACthresh=6.0*0.012;
	...
	for (int round = 0; round < nRANSAC; round++)
	{
		...
		for (int i = 0; i < min_matches; i++) 
		{
			...
			Vx(l_pts[i]) = param[idx].xi0;
			Vy(l_pts[i]) = param[idx].yi0;
			Vz(l_pts[i]) = 1.0;

			Vx(r_pts[i]) = param[idx].xi1;
			Vy(r_pts[i]) = param[idx].yi1;
			Vz(r_pts[i]) = 1.0;
		...
		double rms;
		int num_inliers = CountInliers(param, ptNum,  Mcurr, Outlierth, inliers, rms);
...
// 		}
	...
	double rms;
	std::vector<int> inliers;
	CountInliers(param, ptNum,  Mbest, Outlierth, inliers, rms);
...
// 	}
	...
	int ipos;
	for(i=0, ipos=0; i<inliers.size(); i++, ipos++)
	{
		int j=inliers[i];
		param[j].bAccepted = true;
// 		param->m_ptPairs[ipos].x0 = param->m_ptPairs[j].x0;
// 		param->m_ptPairs[ipos].y0 = param->m_ptPairs[j].y0;
// 		param->m_ptPairs[ipos].x1 = param->m_ptPairs[j].x1;
// 		param->m_ptPairs[ipos].y1 = param->m_ptPairs[j].y1;
	}
...
// 						double *M, double thresh, std::vector<int> &inliers)
{
	...
	for (unsigned int i = 0; i < ptNum; i++)
	{
		...
		* if so, increment count and append i to inliers */
		...
		p[0] = point_pairs[i].xi0;
		p[1] = point_pairs[i].yi0;
		p[2] = 1.0;
		...
		double dx = qx - point_pairs[i].xi1;
		double dy = qy - point_pairs[i].yi1;
