...
//m=P*Xc 物方到像方的投影矩阵
//默认myPtPair中xi0, yi0, zi0为物方坐标，xi1, yi1为像素坐标
bool EstimateTransform_Projection(myPtPair *projBuf, int ptNum, int &refineNum, 
								  _iphCamera *camera, double thresh)
{
	...
	{
		projBuf[i].bAccepted = false;
		// 		int idx1 = matches[i].m_idx1;
		// 		int idx2 = matches[i].m_idx2;
		double xp, yp;

		k1_pts[i] = v3_new(projBuf[i].xi0, projBuf[i].yi0, projBuf[i].zi0);

		camera->ImageCoordinateCorrect(projBuf[i].xi1, projBuf[i].yi1, xp, yp);
//		xp=projBuf[i].xi;
//		yp=projBuf[i].yi;
	...
	{
		projBuf[inliers[i]].bAccepted=true;
