...
bool CScanBinReader::open(char *pszFileName, ScanHdr *header)
{
	if(m_fp)
		fclose(m_fp);
	...
	int Ok=ScanReadHdr(header, m_fp);
	if(Ok==0)
		return false;

	m_npoints=header->PntCnt;
	m_pcount=0;
	m_header=*header;
...
int CScanBinReader::read_point(double &x, double &y, double &z)
{
	ScanPnt Spt;
	ScanRow	Mit;
	int Ok=1;
	...
	while (m_pcount<m_npoints)
	{
		if (m_header.HdrVersion == 20020715) 
		{
			Ok = fread( &Spt, sizeof(ScanPnt), 1, m_fp);
			if (Ok != 1)
				break;

			x=(Spt.Pnt.x-m_header.OrgX)/m_header.Units;
			y=(Spt.Pnt.y-m_header.OrgY)/m_header.Units;
			z=(Spt.Pnt.z-m_header.OrgZ)/m_header.Units;
		}
		else 
		{
			Ok = fread( &Mit, sizeof(Mit), 1, m_fp);
			if (Ok != 1)
				break;
			
			//	Convert point information
			x=(Mit.X-m_header.OrgX)/m_header.Units;
			y=(Mit.Y-m_header.OrgY)/m_header.Units;
			z=(Mit.Z-m_header.OrgZ)/m_header.Units;
