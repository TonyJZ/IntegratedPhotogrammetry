...
//	double tx, ty, tz;  //偏心分量
	...
	orsLidJingParam *pOriParam;	//系统误差初值
...
void distance_PointToPlane_Jing(double *p, double *y, int m, int n, void *data)
{
	register int i, j;
	struct myObs *dptr;
	...
	pLidPoints=dptr->pLidPoints;  //不能改变坐标值
	pMisalign=p;
	...
	{
		pOriParam[i].phi = p[0];
		pOriParam[i].omega = p[1];
		pOriParam[i].kappa = p[2];
...
// 	}
	...
	{//遍历平面
		...
		{//遍历平面内的点
			...
			orsPOINT3D oriPts, dstPts;
			oriPts.X = pLidPoints[j].x;
			oriPts.Y = pLidPoints[j].y;
			oriPts.Z = pLidPoints[j].z;

			pLidModels[sID]->cal_Laserfootprint(&oriPts, &dstPts, 1);
...
//求未知数偏导数
void jac_dis_PointToPlane_Jing(double *p, double *jac, int m, int n, void *data)
{
	register int i, j;
	struct myObs *dptr;
	LidPt_SurvInfo *pLidPoints;
...
//差分方式求导
void jac_dis_PointToPlane_Jing_diff(double *p, double *jac, int m, int n, void *data)
{
	register int i, j;
	struct myObs *dptr;
	LidPt_SurvInfo *pLidPoints;
	...
	//计算当前未知数条件下的方程观测值
	distance_PointToPlane_Jing(p, hx, ncol, nPoints, data);
	...
	for(j=0; j<ncol; ++j)
	{
		...
		distance_PointToPlane_Jing(p, hxx, ncol, nPoints, data);
...
void LidStatDis_PointToPlane_Jing(LidMC_Plane *pLidPatch, int nPatch, orsILidarGeoModel **pLidModels, int nStrip,
	double *Lidparam, bool bInit, orsLidJingParam *pOriParam, double *rms, int nTies, double *pDis)
{
	double *X=NULL;
...
// 		pLidModels[sID]->cal_Laserfootprint(&(*(pLidPatch[i].pLidPoints))[0], pLidPatch[i].pLidPoints->size());
	...
	for(i=0; i<nStrip; i++)
	{
		pLidarModels[i] = ORS_CREATE_OBJECT(orsILidarGeoModel, /*ORS_LIDAR_GEOMETRYMODEL_Yuan*/ORS_LIDAR_GEOMETRYMODEL_JING);
...
//	data.type = param_type;
	...
	//计算点面距离
	distance_PointToPlane_Jing(X, dis, nCalibParam+4*nTies, nLidPoints, &data);
...
//只计算观测值
void cal_Obs_ConjugatePts_Jing(double *p, double *y, int m, int n, void *data)
{
	register int i, j, k;
	struct myObs *dptr;
	LidMC_VP *pLidVP;
...
//		pLidModels[sID]->cal_Laserfootprint(&pLidPoints[i], 1);
		...
		orsPOINT3D oriPts, dstPts;
		oriPts.X = pLidPoints[i].x;
		oriPts.Y = pLidPoints[i].y;
		oriPts.Z = pLidPoints[i].z;
...
void jac_ConjugatePts_Jing(double *p, double *jac, int m, int n, void *data)
{
	register int i, j;
	double tmp;
	register double d;
...
int  CLidCalib_Jing::LidMC_Calib_Coplanar(
...
// 	dPlane=new double[nParam];
	...
	int nCalibParam = 3;     //five unknowns
//	int nMisalign=3;
	X = new double[nCalibParam + 4*nTies];
 	for(i=0; i<nCalibParam; i++)
 	{
...
// 	C=new double[nLidPoints*4*nTies];	//未采用压缩存储
	...
	for(i=0; i<nStrips; i++)
	{
		pLidarModel[i] = ORS_CREATE_OBJECT(orsILidarGeoModel, /*ORS_LIDAR_GEOMETRYMODEL_Yuan*/ORS_LIDAR_GEOMETRYMODEL_JING);
...
/*
	...
	LidStatDis_PointToPlane_Jing(pLidPlane, nPlane, pLidarModel, nStrips, X, false, pOriParam, rms, nTies, NULL);
...
//	fprintf(fp, "## calib param type: %d\n", type);
	...
	int ret=dlevmar_der(distance_PointToPlane_Jing, jac_dis_PointToPlane_Jing, X, L, nCalibParam+nTies*4, nLidPoints, 1000, 
...
// 	}
	...
	LidStatDis_PointToPlane_Jing(pLidPlane, nPlane, pLidarModel, nStrips, X, true, pOriParam, rms, nTies, dis);
...
// 				fprintf(fp, " /");
	...
	pOriParam[0].phi = X[0]; pOriParam[0].omega = X[1]; pOriParam[0].kappa = X[2];
//	pOriParam[0].kappa = X[3]; pOriParam[0].omega = X[4];
	write_CPfile(cParamFile, LCMT_Jing, (void*)pOriParam, 1);
...
int  CLidCalib_Jing::LidMC_Calib_ConjugatePoints(
		const char *pszResultFile,		/* 平差结果文件 */
...
//	POINT3D *ptSrc=NULL, *ptDst=NULL;
	...
	for(i=0; i<nStrips; i++)
	{
		pLidarModel[i] = ORS_CREATE_OBJECT(orsILidarGeoModel,ORS_LIDAR_GEOMETRYMODEL_JING);
...
//	data.L = NULL; //不更新L
	cal_Obs_ConjugatePts_Jing(X, hx, pnp, nObs, &data);
...
//#endif
	...
	int ret=dlevmar_der(cal_Obs_ConjugatePts_Jing,	//计算L
								jac_ConjugatePts_Jing, //计算A
