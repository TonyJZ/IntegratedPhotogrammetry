...
struct myObs
{
	...
	orsLidResslParam *pOriParam;	//系统误差初值
...
void distance_PointToPlane_Ressl(double *p, double *y, int m, int n, void *data)
{
	register int i, j;
	struct myObs *dptr;
	...
	for(i=0; i<nStrip; i++)
	{
		...
		pOriParam[i].ay = p[1];
		pOriParam[i].az = p[2];
		pOriParam[i].aroll = p[3];
		pOriParam[i].ayaw = p[4];
...
//求未知数偏导数
void jac_dis_PointToPlane_Ressl(double *p, double *jac, int m, int n, void *data)
{
	register int i, j;
	struct myObs *dptr;;;
	LidPt_SurvInfo *pLidPoints;
...
//差分方式求导
void jac_dis_PointToPlane_Ressl_diff(double *p, double *jac, int m, int n, void *data)
{
	register int i, j;
	struct myObs *dptr;
	LidPt_SurvInfo *pLidPoints;
	...
	//计算当前未知数条件下的方程观测值
	distance_PointToPlane_Ressl(p, hx, ncol, nPoints, data);
	...
	for(j=0; j<ncol; ++j)
	{
		...
		distance_PointToPlane_Ressl(p, hxx, ncol, nPoints, data);
...
void LidStatDis_PointToPlane_Ressl(LidMC_Plane *pLidPatch, int nPatch, orsILidarGeoModel **pLidModels, int nStrip,
	double *Lidparam, bool bInit, orsLidResslParam *pOriParam, double *rms, int nTies, double *pDis)
{
	double *X=NULL;
...
// 		pLidModels[sID]->cal_Laserfootprint(&(*(pLidPatch[i].pLidPoints))[0], pLidPatch[i].pLidPoints->size());
	...
	for(i=0; i<nStrip; i++)
	{
		pLidarModels[i] = ORS_CREATE_OBJECT(orsILidarGeoModel, ORS_LIDAR_GEOMETRYMODEL_RESSL);
...
//	data.type = param_type;
	...
	//计算点面距离
	distance_PointToPlane_Ressl(X, dis, nCalibParam+4*nTies, nLidPoints, &data);
...
//只计算观测值
void cal_Obs_ConjugatePts_Ressl(double *p, double *y, int m, int n, void *data)
{
	register int i, j, k;
	struct myObs *dptr;
	LidMC_VP *pLidVP;
...
void jac_ConjugatePts_Ressl(double *p, double *jac, int m, int n, void *data)
{
	register int i, j;
	double tmp;
	register double d;
...
CLidCalib_Ressl::CLidCalib_Ressl()
{
	m_covar = NULL;
	m_rms = 0;
	m_corcoef = NULL;
...
int  CLidCalib_Ressl::LidMC_Calib(
...
// 	C=new double[nLidPoints*4*nTies];	//未采用压缩存储
	...
	for(i=0; i<nStrips; i++)
	{
		pLidarModel[i] = ORS_CREATE_OBJECT(orsILidarGeoModel, ORS_LIDAR_GEOMETRYMODEL_RESSL);
...
/*
	...
	LidStatDis_PointToPlane_Ressl(pLidPlane, nPlane, pLidarModel, nStrips, X, false, pOriParam, rms, nTies, NULL);
...
//	fprintf(fp, "## calib param type: %d\n", type);
	...
	int ret=dlevmar_der(distance_PointToPlane_Ressl, jac_dis_PointToPlane_Ressl, X, L, nCalibParam+nTies*4, nLidPoints, 1000, 
...
// 	}
	...
	LidStatDis_PointToPlane_Ressl(pLidPlane, nPlane, pLidarModel, nStrips, X, true, pOriParam, rms, nTies, dis);
...
// 				fprintf(fp, " /");
	...
	write_CPfile(cParamFile, LCMT_Ressl, (void*)pOriParam, 1);
...
int  CLidCalib_Ressl::LidMC_Calib_ConjugatePoints(
		const char *pszResultFile,		/* 平差结果文件 */
...
//	data.L = NULL; //不更新L
	cal_Obs_ConjugatePts_Ressl(X, hx, pnp, nObs, &data);
...
//#endif
	...
	int ret=dlevmar_der(cal_Obs_ConjugatePts_Ressl,	//计算L
								jac_ConjugatePts_Ressl, //计算A
...
// 				fprintf(fp, " -");
	...
	pOriParam[0].aroll = X[3]; pOriParam[0].ayaw = X[4];
	write_CPfile(cParamFile, LCMT_Ressl, (void*)pOriParam, 1);
