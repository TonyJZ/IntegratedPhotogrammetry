...
//m=P*Xc 物方到像方的投影矩阵
bool EstimateTransform_Projection(Align_Keypoint *pBuf1, Align_Keypoint *pBuf2, int ptNum, int &refineNum, 
								  _iphCamera *camera, double thresh)
{
	...
	for (int i = 0; i < num_pts; i++) 
	{
		...
		k1_pts[i] = v3_new(pBuf1[i].pt3D.X, pBuf1[i].pt3D.Y, pBuf1[i].pt3D.Z);
...
//		camera->Image2Photo(pBuf2[i].pt2D.x, pBuf2[i].pt2D.y, xp, yp);
	...
	{
		pBuf1[inliers[i]].flag=keypoint_inlier;
