#ifndef _ORS_LIDARGEOMETRY_MODEL_Jing_INCLUDE_H_
#define _ORS_LIDARGEOMETRY_MODEL_Jing_INCLUDE_H_
...
class orsXLidarGeoModel_Jing:  public orsILidarGeoModel, public orsObjectBase
{
private:
	//局部坐标系定义为XYZ -->  东北天
//	double		m_rotZ;				//条带绕Z轴的旋转角
	double		m_rotMatZ[9];		//条带绕Z轴的旋转矩阵
	double		m_rotMatZ_T[9];		//转置
	...
	orsLidJingParam	*m_extParam;

private:
	//用于辅助计算的变量
	double		m_XCentered[3];  //中心化
	double		m_XStriped[3];	//条带坐标系
//	int			m_vecLen;
...
public:
	orsXLidarGeoModel_Jing(bool bForRegister);
	virtual ~orsXLidarGeoModel_Jing();
...
public:
	ORS_OBJECT_IMP1( orsXLidarGeoModel_Jing, orsILidarGeoModel, "Jing", "Jing" );
};
